    ...
    init: function () {
        ...
        this.addFunItem = function (funInfo) {
            if (funInfo) {
                ...
                if (!funPanel || funInfo.funViewXtype == "serviceitem") {
                    if (null != myMask)
                        myMask.show();
                    try {
                        ...
                        if (funInfo.funViewXtype == "serviceitem") {
                            funPanel.html = '<iframe id="frame" src="serviceview/service.html?' + funInfo.fid + '&' + funInfo.flowname + '" frameborder="0" width="100%" height="100%"></iframe>';
                        }
                    } catch (e) {
                        myMask.hide();
                    }
                ...
                "westview treepanel": {
                    itemclick: function (tree, record, item, index, e, eOpts) {
                        ...
                        if (id.startsWith("fid")) {
                            self.addFunItem({
                                mainView: mainView,
                                funViewXtype: "serviceitem",
                                funController: "core.service.controller.ServiceController",
                                funViewName: "core.service.view.ServiceItem",
                                fid: id.split(":")[1],
                                flowname: record.data["text"]
                            });
                            return
