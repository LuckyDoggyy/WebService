 ...
 * subclass of {@link Ext.layout.container.Fit fit layout}, CenterLayout
 * expects to have one child item; multiple items will be placed overlapping.
 * The layout does not require any config options. Items in the container can
 * use percentage width or height rather than be fit to the full size of the
 * container.
 * 
 * Example usage:
 *  // The content panel is centered in the container
 * 
 * var p = Ext.create('Ext.Panel', { title: 'Center Layout', layout:
 * 'ux.center', items: [{ title: 'Centered Content', width: '75%', // assign 75%
 * of the container width to the item html: 'Some content' }] });
 * 
 * If you leave the title blank and specify no border you can create a
 * non-visual, structural container just for centering the contents.
 * 
 * var p = Ext.create('Ext.Container', { layout: 'ux.center', items: [{ title:
 * 'Centered Content', width: 300, height: '90%', // assign 90% of the container
 * height to the item html: 'Some content' }] });
 */
Ext.define('Ext.ux.layout.Center', {
	...
	getPos : function(itemContext, info, dimension) {
		var size = itemContext.props[dimension] + info.margins[dimension], pos = Math
