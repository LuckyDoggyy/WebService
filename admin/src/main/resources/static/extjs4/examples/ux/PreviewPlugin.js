...
Ext.define('Ext.ux.PreviewPlugin', {
	...
	/**
	 * @cfg {String} bodyField Field to display in the preview. Must be a field
	 *      within the Model definition that the store is using.
	 */
	bodyField : '',
		...
		var bodyField = this.bodyField, hideBodyCls = this.hideBodyCls, features = [
				{
					...
					getAdditionalData : function(data, idx, record, orig, view) {
						...
						if (getAdditionalData) {
							Ext.apply(additionalData, getAdditionalData.apply(
											this, arguments));
						}
	 ...
	 * 
	 * @param {Boolean}
	 *            expanded Pass true to expand the record and false to not show
	 *            the preview.
	 */
