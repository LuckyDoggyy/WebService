...
public class KuhnMunkres {
    ...
    public static int n;
    public static double [][] value = new double[MAX][MAX];   //给定二分图的权重值
    public static double [] xWeight = new double [MAX];
    public static double[] lx = new double[MAX];   //记录二分图左半部分顶点的可行顶标
    public static double[] ly = new double[MAX];   //记录二分图右半部分顶点的可行顶标
    ...
    public static void setxWeight(double[] xWeight) {
        KuhnMunkres.xWeight = xWeight;
    }

    public double getKM(int judge) {
        ...
        for(int y = 0;y < n;y++) {
            if(pre[y] != -1)
                sum = sum + xWeight[pre[y]] * value[pre[y]][y];
        }
        if(judge == -1)
            sum = -1 * sum;
        return sum;
